...
setup_tmp() {
    TMP_DIR=$(mktemp -d -t rageta-install.XXXXXXXXXX)
    TMP_METADATA="${TMP_DIR}/rageta.json"
    TMP_HASH="${TMP_DIR}/rageta.hash"
    TMP_BIN="${TMP_DIR}/rageta.tar.gz"
    cleanup() {
        local code=$?
        set +e
        trap - EXIT
...
get_release_version() {
    if [[ -n "${RAGETA_VERSION}" ]]; then
      SUFFIX_URL="tags/v${RAGETA_VERSION}"
    else
      SUFFIX_URL="latest"
    fi
    ...
    VERSION_RAGETA=$(grep '"tag_name":' "${TMP_METADATA}" | sed -E 's/.*"([^"]+)".*/\1/' | cut -c 2-)
    if [[ -n "${VERSION_RAGETA}" ]]; then
        info "Using ${VERSION_RAGETA} as release"
    else
...
download_hash() {
    HASH_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION_RAGETA}/rageta_${VERSION_RAGETA}_checksums.txt"
    # NB: support the checksum filename format prior to v0.6.0
    set +e
    vercomp ${VERSION_RAGETA} 0.6.0
    if [[ $? -eq 2 ]]; then
        HASH_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION_RAGETA}/rageta_${VERSION_RAGETA}_checksums.txt"
    fi
    set -e
    ...
    HASH_EXPECTED=$(grep " rageta_${VERSION_RAGETA}_${OS}_${ARCH}.tar.gz$" "${TMP_HASH}")
    HASH_EXPECTED=${HASH_EXPECTED%%[[:blank:]]*}
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION_RAGETA}/rageta_${VERSION_RAGETA}_${OS}_${ARCH}.tar.gz"
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing rageta to ${BIN_DIR}/rageta"
    tar -xzof "${TMP_BIN}" -C "${TMP_DIR}"

    local CMD_MOVE="mv -f \"${TMP_DIR}/rageta\" \"${BIN_DIR}\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"

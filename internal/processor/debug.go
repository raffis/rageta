...
func NewProxy(logger logr.Logger, debug bool, spec *v1beta1.Step, processor Bootstraper, processors ...Bootstraper) []Bootstraper {
	if !debug {
		return processors
	}
...
func (s *Proxy) Bootstrap(pipeline Pipeline, next Next) (Next, error) {
	wrappedNext, err := s.wrapped.Bootstrap(pipeline, next)
	if err != nil {
		return next, err
	}

...
func (p *textMux) printVtx(t *trace, dgst digest.Digest) {
	...
	if dgst != p.current {
		if p.current != "" {
			...
			old.logsOffset = 0
			old.count = 0
			fmt.Fprintf(p.events, "#%d ...\n", old.index)
		}

		if p.notFirst {
			fmt.Fprintln(p.events, "")
		} else {
			if p.desc != "" {
				fmt.Fprintf(p.events, "#0 %s\n\n", p.desc)
			}
			p.notFirst = true
		}

		if os.Getenv("PROGRESS_NO_TRUNC") == "0" {
			fmt.Fprintf(p.events, "#%d %s\n", v.index, limitString(v.Name, 72))
		} else {
			fmt.Fprintf(p.events, "#%d %s\n", v.index, v.Name)
		}
	}
	...
	if v.isCompleted() && !isOpenStatus {
		...
		if v.Error != "" {
			if strings.HasSuffix(v.Error, context.Canceled.Error()) {
				fmt.Fprintf(p.events, "#%d CANCELED\n", v.index)
			} else {
				fmt.Fprintf(p.events, "#%d ERROR: %s\n", v.index, v.Error)
			}
		} else if v.Cached {
			fmt.Fprintf(p.events, "#%d CACHED\n", v.index)
		} else {
			tm := ""
			var ivals []interval
